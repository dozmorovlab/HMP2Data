...
And patient level characteristics can be presented as so.
...
library(gridExtra)
# set up ggplots
p1 <- ggplot(momspi16S_samp, aes(x = visit_number)) + geom_histogram(bins = 20, fill = "#00BFC4") +
  theme(panel.background = element_blank(), panel.grid = element_blank()) + xlim(c(0,20)) + xlab("visit number")
p1
```

Here we plot the histogram of the number of samples at each visit for the other studies included in the package. 
...
p2 <- ggplot(plot_visits, aes(x = visits, fill = study)) + geom_histogram(position = "dodge", alpha = 0.7, bins = 40) + xlim(c(0,40)) +
  theme(legend.position = c(0.8, 0.9), panel.background = element_blank(), panel.grid = element_blank())  + xlab("visit number")
p2
```
...
# plot
upset(momspi_upset, order.by = 'freq')
```
...
# plot
upset(momspiCyto_upset, order.by = 'freq')
```
