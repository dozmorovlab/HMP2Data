...
Here we plot histograms of the number of samples at each visit for the MOMS-PI 16S data.

```{r}
library(ggplot2)
library(cowplot)

ggplot(momspi16S_samp, aes(x = visit_number)) + geom_histogram(bins = 67)
```

Now for the MOMS-PI Cytokines data.

```{r}
ggplot(momspiCyto_samp, aes(x = visit_number)) + geom_histogram(bins = 15)
```

For the IBD data.

```{r}
ggplot(IBD16S_samp, aes(x = visit_number)) + geom_histogram(bins = 24)
```

And finally for the T2D data.

```{r, warning=FALSE}
# First remove unknown values with 999 code
T2D16S_samp$visit_number[T2D16S_samp$visit_number == 999] <- NA
ggplot(T2D16S_samp, aes(x = visit_number)) + geom_histogram(bins = 200)
```
