...
abstract: >
    ...
    Human Microbiome Project (iHMP), also known as the second phase of the HMP 
    ...
    inflammatory bowel disease; and 3) T2D, the onset of type 2 diabetes and 
...
The demographic data contained in `HMP2Data` can be summarized using the `table_two()` function. First, we need to make a list of the `phyloseq` and `SummarizedExperiment` object which can then be entered into the table generating function. "N" and "%" are the number and percent of subjects with a given annotation, respectively.
...
We can also create a table of the break down of samples by visit number quantile. That is, the first quantile will contain visit counts belonging to the first quantile of the whole range of visit numbers. The split of visits by quantiles helps to account for the fact that different studies have different ranges of visits.
...
<!-- The MOMS-PI 16S data contains samples from `r length(unique(sample_data(momspi16S_phyloseq)$sample_body_site))` body sites. The number of samples from each site can be displayed as follows. The subjects were all female, and the race was unknown for every participant. -->
