...
The MOMS-PI 16S data contains samples from `r length(unique(sample_data(momspi16S_phyloseq)$sample_body_site))` body sites. The number of samples from each site can be displayed as follows.

```{r}
table(sample_data(momspi16S_phyloseq)$sample_body_site, sample_data(momspi16S_phyloseq)$visit_number) %>% t()
```
...
```{r}
table(colData(momspiCyto)$sample_body_site, colData(momspiCyto)$visit_number) %>% t()
```
...
```{r}
table(sample_data(T2D)$sample_body_site, sample_data(T2D)$visit_number) %>% t()
```
