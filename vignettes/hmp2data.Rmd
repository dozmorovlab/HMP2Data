...
Once installed `HMP2Data` provides functions to access the HMP2 data. When called, as follows, the funcctions will download data from an `ExperimentHub` Amazon S3 (Simple Storage Service) bucket over `https` or load data from a local cache.
...
Here we have a `phyloseq` object containing the 16S rRNA seq data for 7,665 taxa and 9,107 samples. The MOMS-PI cytokine data is stored in a `SummarizedExperiment` object and can be loaded as follows.
...
The MOMS-PI 16S data contains samples from 5 body sites. The number of samples from each site can be displayed as follows.
...
The MOMS-PI cytokine data contains samples from 3 body sites.
