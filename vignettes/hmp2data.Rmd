...
Load 16S data, rows are Greengene IDs, columns are samples:
...
Load the Greengenes taxonomy table, rows are Greengene IDs, columns are taxonomic ranks:
...
Load the sample annotation data, rows are samples, columns are annotations:
...
Here we have a `phyloseq` object containing the 16S rRNA seq data for `r nrow(otu_table(momspi))` taxa and `r ncol(otu_table(momspi))` samples. 

The MOMS-PI cytokine data is stored in a `SummarizedExperiment` object and can be loaded as follows.
