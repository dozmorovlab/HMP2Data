...
dataDir <- "data.IBD/ibd/" # path to folder contianing data files
...
files <- list.files("data.IBD/ibd") # data files
...
for(i in 1:length(files)) {
  res <- try(phylo_list[[i]] <- import_biom(paste0(dataDir, files[i])), TRUE)
...
# RE-Read only the files that are readable
load('data/ibd_readable_files.rda') # readable files
for(i in 1:length(files)) {
  res <- try(phylo_list[[i]] <- import_biom(paste0(dataDir, files[i])), TRUE)
  if(isTRUE(class(res)=="try-error")) { next } else {phylo_list[[i]] <- res}
  # print(i)
